...
class ServerCoordinator(
    ...
    fun gatewayRoutes(): GatewayRoutes =
        GatewayRoutes(
            ...
            js5 = GatewayRoute(js5::serve),
            login = GatewayRoute(::loginAndTransfer),
        )
    ...
    private suspend fun loginAndTransfer(
        ...
        val reservation =
            withTimeoutOrNull(config.worldEntryTimeout) { game.reserve(session.account.accountId) }
        ...
        val attached =
            game.enter(
                read,
                write,
                ConnectionHandoff(session.account, session.isaac, accepted),
            ) { playerIndex ->
                check(playerIndex == accepted.playerIndex) { "game service changed the reserved player index" }
