...
class GameInboundReaderTest {
    ...
    fun `unsupported packet is framed before move click without losing ISAAC alignment`() = runBlocking {
        ...
        val actions = GameInputQueue(GameInputQueueConfig())
        ...
        val route = actions.drainToList().single() as PlayerAction.Route
    ...
    private fun GameInputQueue.drainToList(): List<PlayerAction> =
        buildList { drain(::add) }
}
