 ...
 *  - **op16 fresh login** (`cd.az` -> `cd.am`): after response code 2 the client waits for the
 *    index-dependent trailer, which the game stage sends only after world admission.
...
class ReconnectLoginTest {
    ...
    @Test fun `fresh op16 authentication gets response 2 before world admission supplies its trailer`() {
        val keyPair = loadRealOrSkip() ?: return
        assertContentEquals(byteArrayOf(2), loginResponseBytes(keyPair, reconnect = false))
    }
