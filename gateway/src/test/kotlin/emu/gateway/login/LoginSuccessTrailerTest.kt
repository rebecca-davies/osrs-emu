...
class LoginSuccessTrailerTest {
    @Test fun `trailer advertises account info plus the first game header without padding it`() {
        val trailer = loginSuccessTrailer(PlayerRank.PLAYER, playerIndex = 1)
    ...
    @Test fun `di field encodes the admitted player index big-endian`() {
        val trailer = loginSuccessTrailer(PlayerRank.PLAYER, playerIndex = 1_736)
    ...
    @Test fun `every byte outside the di field is zero`() {
        val trailer = loginSuccessTrailer(PlayerRank.PLAYER, playerIndex = 2_047)
    ...
    @Test fun `player index must fit a non-zero rev-239 player slot`() {
        kotlin.test.assertFailsWith<IllegalArgumentException> {
            loginSuccessTrailer(PlayerRank.PLAYER, playerIndex = 0)
        }
        kotlin.test.assertFailsWith<IllegalArgumentException> {
            loginSuccessTrailer(PlayerRank.PLAYER, playerIndex = 2_048)
        }
    }
}
