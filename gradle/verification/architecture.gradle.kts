    ...
    doLast {
        ...
        for ((path, permitted) in allowed) {
            ...
            check(actual == permitted) {
                "$path project dependencies differ: missing=${permitted - actual}, " +
        ...
        val apiForbidden =
            Regex(
                "^import (java\\.sql|javax\\.sql|at\\.favre|emu\\.server|emu\\.protocol)",
        ...
        val postgresForbidden =
            Regex(
                "^import (at\\.favre|emu\\.server|emu\\.protocol)",
                RegexOption.MULTILINE,
            )
        ...
        val loginAuth =
            requireNotNull(findProject(":server-login"))
                .projectDir
                .resolve("src/main/kotlin/emu/server/login/auth")
        ...
        for ((path, ownPackage) in servicePackages) {
            ...
            val sources =
                service.projectDir.resolve("src").walkTopDown()
                    .filter { it.isFile && it.extension == "kt" }
            for (source in sources) {
                ...
                for (peerPackage in servicePackages.values - ownPackage) {
                    check(!text.contains("import $peerPackage")) {
                        "$path imports peer service package in $source"
                    }
                }
            }
        }
