...
class CharacterSaveWriterTest {
            ...
            CharacterSaveWriter(
                store =
                    characterStore { value ->
                        if (value.characterId == 1L) {
                            firstStarted.countDown()
                            releaseFirst.await()
                        }
                        saved += value.characterId
                    },
    ...
    fun `close drains every accepted save before returning`() {
        ...
        val writer =
            CharacterSaveWriter(
                store = characterStore { saved += it.characterId },
            ...
            CharacterSaveWriter(
                store =
                    characterStore { value ->
                        ...
                        saved += value.characterId
                        if (value.characterId == 2L) secondSaved.countDown()
                    },
                config =
    ...
    private fun characterStore(write: (CharacterSave) -> Unit): CharacterStore =
        object : CharacterStore {
            ...
            override fun save(save: CharacterSave) = write(save)
        }
}
