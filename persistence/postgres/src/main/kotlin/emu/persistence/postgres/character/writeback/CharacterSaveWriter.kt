...
class CharacterSaveWriter(
    ...
    private val pendingCharacterIds = ConcurrentHashMap.newKeySet<Long>()
    ...
    override fun submit(save: CharacterSave): CharacterWriteCompletion? =
        synchronized(lifecycle) {
            if (!accepting.get()) return@synchronized null
            if (!capacity.tryAcquire()) return@synchronized null
            if (!pendingCharacterIds.add(save.characterId)) {
                capacity.release()
    ...
    private fun finish(pending: PendingSave) {
        check(pendingCharacterIds.remove(pending.save.characterId)) {
            "pending character save was not tracked"
        }
        capacity.release()
    }
    ...
    private inner class PendingSave(val save: CharacterSave) : CharacterWriteCompletion, Delayed {
